...
4. Resolve the exact framework repository URL and full commit recorded in PROJECT.
   Stop if that version is unavailable, mismatched, or has local changes.
5. Read `SKILL.md` and `references/PROTOCOL.md` from that framework version.
6. Load only the charter required for the next role.
7. Existing host and project instructions still apply. Stop and report a conflict that
...
- the recorded source URL and full commit resolve to a readable GLBuilding pack;

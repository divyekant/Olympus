...
| frontend commit and hook checkpoint | `<material frontend only: exact pre-commit intended included-path identity, named-path commit, exact post-hook committed HEAD/index/working-tree identities, their hook delta, owning-role validation, whether validated bytes were already cleanly committed or needed the one corrective commit, cumulative identity refreshed from final committed HEAD bytes, and corrective-commit result or blocked outcome>` |

...
| D027 | Provider-neutral Release Agent boundary | The current 15-role catalog includes a Release Agent for owner-requested preparation, remote reconciliation, or one release-boundary action. The [release boundary](../references/PROTOCOL.md#release-boundary) keeps source-only preparation separate from immutable execution, requires exact owner approval and verified consumption, gives the role no file or standing external authority, and permits at most one provider action submission per execution handoff. Read-only provider evidence calls do not consume that allowance. The contract does not claim live provider or production support. |

...
| D034 | Task-related zero-P2 convergence by default | `strict convergence` defaults `on`: the specification bracket's qualifying round requires zero open task-related P0-P2, matching the cap rule, which already required zero open P0-P2 regardless of the setting. An owner may relax the qualifying-round bar to P0/P1 only by setting the boundary `off`; value meanings are unchanged. A P2 or P3 finding is task-related when its minimum reproducing evidence names a clause in the current specification body, a path in the goal's allowed paths, or an artifact the owner request bytes name; otherwise it is non-essential. The Orchestrator classifies each P2 and P3, and files every non-essential one as a repository issue, recording the issue reference beside the finding, instead of carrying it as accepted residue in the task record. Filing failure keeps the finding open and blocking, recorded as skipped or unrunnable work with a recovery owner; nothing is silently dropped. |

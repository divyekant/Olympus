...
29. Current PROJECT or current-repository evidence may provide the complete frontend source
    ...
    records the exact pre-commit Git state and compares its intended path identity with post-hook committed HEAD, index, and
    working-tree identities. The owning role validates each changed path. A corrective commit runs
...
| Commit/evidence order | Builder implementation returns `frontend evidence packet: not yet permitted`; Builder edits and tests first; Docs Writer runs only when triggered; the Orchestrator records the exact pre-commit Git state and intended path identity, commits named paths, completes hooks, and records post-hook committed HEAD, index, and working-tree identities. Each difference returns to its owning role for validation. If validated bytes are already cleanly committed, no empty repair commit runs; otherwise exactly one corrective commit is allowed, and a post-correction difference blocks. The proper cumulative identity refreshes from final committed HEAD bytes. The evidence-only Builder pass generates evidence only against that exact clean commit; paired fresh reviewers use the same commit; after paired pass, final checks are non-generating only. Any relevant change invalidates both verdicts. |
